...
app.get("/comics", (req, res) => {
    db.query(`SELECT id, title, illustrator, author, year_start, resume, picture from comic`, (errSql, resSql) => {
        if (errSql) {
...
app.get("/comics/:id", (req, res) => {
    ...
    db.query(`SELECT id, title, illustrator, author, year_start, resume, picture from comic WHERE id=?`, [id], (errSql, resSql) => {
        if (errSql) {
            return res.status(500).send("Nope, sorry, I iz broken !")
        }
        if (!resSql.length) {
